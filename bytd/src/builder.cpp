...
public:
    explicit PrevetranieReku(Reku& reku)
        : reku(reku)
    {
    }
...
void Builder::buildMisc(slowswi2cpwm& ioexpander, OpenTherm& ot)
{
    ...
    components.reku = std::make_unique<Reku>(mqtt, "/dev/ttyS4");
    ...
    assignSensor(kurenie::ROOM::Kupelka, findSensor("KupelnaT"));
...
struct CanNodeInfo {
    explicit CanNodeInfo(std::string name, unsigned id)
        : name(name)
        , id(id)
    {
