...
public:
    ...
    LSubscription(const Target& target)
        : mTarget(target)
    {
    }
...
namespace maXYmo_priv {
    ...
    template <bool CallbackConstness, class TListener, class TResult, typename... TArgs>
    struct CallbackType {
    };

    template <class TListener, class TResult, typename... TArgs>
    struct CallbackType<true, TListener, TResult, TArgs...> {
        typedef TResult (TListener::*Target)(TArgs...) const;
    };

    template <class TListener, class TResult, typename... TArgs>
    struct CallbackType<false, TListener, TResult, TArgs...> {
