...
MonoStableDev::MonoStableDev(std::unique_ptr<IDigiOut> out, std::string name, std::shared_ptr<MqttClient> mqtt, boost::asio::io_service& io_context, float timeout_sec)
    :DeviceBase(std::move(out), name, mqtt)
    ,timer(io_context)
